import log4js, { type Appender, type Log4js } from 'log4js'
...
const defaultLogger = localLog4js?.getLogger('normal')
const errorLogger = localLog4js?.getLogger('error')

if (isDev) {
  defaultLogger && defaultLogger.level = 'debug'
  errorLogger && errorLogger?.level = 'debug'
} else if (isProd) {
  defaultLogger&&defaultLogger.level = 'error'
...
export const stdoutLogger = localLog4js?.getLogger('stdout')
export const logger = {
  info,
  warn,
  debug,
