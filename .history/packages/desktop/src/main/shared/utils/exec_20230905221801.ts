...
export const customExec = async (
  ...
  const spawnPromise = new Promise<[ChildProcess | undefined, boolean]>((resolve) => {
    ...
    run.stdout?.on('data', (data) => {
      logger.info('customExec stdout', `${data.toString().trimEnd()}`)
      out += data.toString()
    })

    run.stderr?.on('data', (data) => {
      logger.info('customExec stderr', `${data.toString().trimEnd()}`)
    })

    run.on('close', function (code) {
      if (code === 0) {
        logger.info('customExec close', `child process exited 执行正常`)
        resolve([run, true])
      } else {
        logger.info('customExec close', `child process exited 执行失败 ${code?.toString()}`)
