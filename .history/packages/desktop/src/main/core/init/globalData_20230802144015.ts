...
async function writeBackendConfig() {
  backendConfig = await readBackendConfig()
  backendConfig = backendConfig.Port = Number(global.shareState.backend.port)
  backendConfig.DevServer.Port = Number(global.shareState.backend.healthPort)
  backendConfig.Sqlite.DbFilePath = global.shareState.backend.sqlite.dbFilePath
  backendConfig.save()
}
