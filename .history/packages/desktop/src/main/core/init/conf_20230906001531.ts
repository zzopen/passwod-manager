...
export const makeDir = async () => {
  ...
  paths = [defaultConfig.dataDirPath, defaultConfig.backend.dirPath, defaultConfig.desktop.dirPath]
  // 确保数据根目录存在
  ...
  if (defaultConfig.backend.dirPath) {
    console.log(888)
    await fse.ensureDir(defaultConfig.backend.dirPath, options)
  }

  if (defaultConfig.desktop.dirPath) {
    console.log(777)
    await fse.ensureDir(defaultConfig.desktop.dirPath, options)
  }
