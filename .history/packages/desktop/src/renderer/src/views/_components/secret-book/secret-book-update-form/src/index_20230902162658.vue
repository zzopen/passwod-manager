...
const rulesRef = reactive({
  ...
  secret_category_id: [{ required: false, message: '请选择分类' }]
})
...
const onSubmit = async () => {
  try {
    await validate()
    const isSuccess = await requestStore.updateSecretBook(unref(dataSourceRef))
    if (!isSuccess) {
      return false
    }
  ...
  <div>
    <a-form ref="formRef" :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
      ...
      <a-form-item label="分类" v-bind="validateInfos.secretCategoryId">
        <a-select
          v-model:value="dataSourceRef.secret_category_id"
          show-search
          allow-clear
          placeholder="请选择分类"
          style="width: 200px"
