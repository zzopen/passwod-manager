...
import { SecretBookList, SecretBookListItem } from './props'
...
const dataSource = ref<SecretBookList>([])
const ready = async (p: { secret_category_id: string }) => {
  ...
  const tempDataSource: SecretBookList = []
  for (const item of secretBookStore.apiSecretBookList) {
    const temp: SecretBookListItem = {
      id: item.id,
      title: item.title
    }
