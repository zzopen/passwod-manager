...
import { addSecretCategoryFormProps } from './props'
...
const props = defineProps(addSecretCategoryFormProps())
...
const dataSourceRef = reactive({ pid: '', name: '' })
console.log(unref(dataSourceRef))
const rulesRef = reactive({
  pid: [{ required: false, message: '请选择父级' }],
  name: [{ required: true, message: '请输入分类名称' }]
})

const { modelRef, resetFields, validate, validateInfos } = useForm(dataSourceRef, rulesRef)
const onSubmit = async () => {
  console.log('modelRef:', modelRef)
  console.log('dataSourceRef:', dataSourceRef)
  console.log('aa:', dataSourceRef == modelRef)
  return false
  // try {
  //   await validate()
...
const submitForm = async () => {
  // 表单提交
  const formData: CreateSecretCategoryRequest = unref(dataSourceRef)
