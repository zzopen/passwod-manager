...
import { type DataSource, addCategoryFormProps, defaultDataSource } from './props'
...
const options = computed(() => {
  console.log(
    'secretCategoryStore.categorySelectOptions:',
    secretCategoryStore.categorySelectOptions
  )
...
const formRef = ref()
const dataSourceRef: DataSource = shallowReactive({ ...defaultDataSource })
...
const resetForm = () => {
  ...
  consoleLog('resetForm:', unref(dataSourceRef))
}

const setFormState = (state: DataSource) => {
  dataSourceRef = Object.assign(dataSourceRef, { ...state })
}
