...
import { logger } from '@renderer/shared'
...
export class AxiosRequest {
  ...
  private setRequestInterceptor() {
    this.instance.interceptors.request.use(
      ...
      (error: any) => {
        // 可以使用全局组件进行提示
        logger.info(error, 'request use error')
  ...
  private setResponseInterceptor() {
    this.instance.interceptors.response.use(
      ...
      (error: any) => {
        ...
        logger.info(message, 'response error msg:')
        return Promise.reject(error)
      }
    )
  }
  ...
  public async request<T = any>(
    ...
    if (method == 'get' || method == 'GET') {
      ...
      const error = new Error(`不支持的请求方式${method}`)
      logger.info(`1--${error}`, 'request')
    ...
    try {
      return methodDo
        .then((response) => {
          ...
          return { error: null, data: apiResult.data, mark: true }
        })
        .catch((error) => {
          logger.info(`2--${error}`, 'request')
          return { error, data: null, mark: false }
        })
    } catch (error) {
      logger.info(`3--${error}`, 'request')
      return Promise.resolve({ error, data: null, mark: false })
    }
  }
}
