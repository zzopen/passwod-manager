...
// add-category-modal
function emitOpenAddCategoryModal(p: { id: number | string }) {
  emitter.emit('openAddCategoryModal', p)
}

function listenerOpenAddCategoryModal(callback: (p: { id: number | string }) => void) {
